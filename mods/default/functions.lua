...
function find_base_pos(xx,yy,zz)
   ...
   -- minetest.forceload_block(pos)
    ...
    local minp,maxp = vm:read_from_map({x=xx,y=yy-10,z=zz},{x=xx,y=yy+40,z=zz})
...
minetest.register_abm({
    ...
    action = function(pos, node)
        ...
        if name == "default:dirt" or name == "default:dirt_with_grass" then
            ...
            pos.y = pos.y+1
            local height = 0
            while minetest.get_node(pos).name == "default:sugar_cane" and height < 4 do
                height = height+1
                pos.y = pos.y+1
            end
            if height < 4 then
                if minetest.get_node(pos).name == "air" then
                    minetest.set_node(pos, {name="default:sugar_cane"})
                end
            end
        end
    end,
        ...
        if not do_preserve then
            ...
            for _, itemname in ipairs(itemstacks) do
                ...
                if minetest.get_item_group(n0.name, "leafdecay_drop") ~= 0 or
                    ...
                    local obj = minetest.add_item(p_drop, itemname)
                    if obj then
