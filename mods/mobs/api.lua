...
function hit(self)
    local olkdtextures = self.textures
	prop = {
		mesh = mummy_mesh,
		textures = {self.textures[1].."^pyramids_hit.png"},
	}
	self.object:set_properties(prop)
	minetest.after(0.4, function()
		prop = {textures = oldtextures,}
		self.object:set_properties(prop)
	end)
end
			...
			if hitter and hitter:is_player() and hitter:get_inventory() then
				for _,drop in ipairs(self.drops) do
					if math.random(1, drop.chance) == 1 then
					   local obj = minetest.env:add_item(self.object:getpos(), drop.name.." "..math.random(drop.min, drop.max))						
					end
				end
			end
		end
