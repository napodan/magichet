...
minetest.register_node("flowers:moonflower_closed", {
	...
	paramtype = "light",
	walkable = false,
	groups = { snappy = default.dig.wool, flammable=2, flower=1, wield_light=2 },
...
minetest.register_node("flowers:moonflower_open", {
	...
	groups = { not_in_creative_inventory = 1, snappy = default.dig.leaves, flammable=2, flower=1, wield_light=4 },
	drop = 'flowers:moonflower_closed',
	sounds = default.node_sound_leaves_defaults(),
	selection_box = {
		type = "fixed",
...
set_moonflower = function (pos)
	...
	if ((minetest.get_timeofday() < OPEN_TIME_START) or (minetest.get_timeofday() > OPEN_TIME_END)) then
