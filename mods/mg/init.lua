...
minetest.register_on_mapgen_init(function(mgparams)
        wseed = mgparams.seed --math.floor(mgparams.seed/10000000000)
end)
...
local function mg_generate(minp, maxp, emin, emax, vm)
        ...
        if fnd then
           ...
           print("A village spawned at: x="..vx..", z="..vz..", y="..vh)
           table.insert(vvvg,{vx=vx,vz=vz})
        end
        --print("A village spawned at: x="..vx..", z="..vz)
...
-----------------------------------------------------------------------------------
           --[[   local result = find_base_pos(x,-20,z)
                local top_nn
                local c_top_id
                if result then
                    top_nn = minetest.get_node(result).name
                    if top_nn then
                       c_top_id = minetest.get_content_id(top_nn)
                    else
                       c_top_id = minetest.get_content_id('default:dirt')
                    end
                end]]--
------------------------------------------------------------------------------------
                ni = ni + 1
...
--[[
        ...
        local to_add = generate_village(vx, vz, vs, vh, minp, maxp, data, a, village_noise, villages_to_grow) or {}

        vm:set_data(data)
print('data set for '..minetest.pos_to_string({x=vx,y=vh,z=vz}))
        vm:calc_lighting(
...
--minetest.after(1,function(dtime)
        ...
        local vm,emin,emax = minetest.get_mapgen_object('voxelmanip') --minetest.get_voxel_manip()
