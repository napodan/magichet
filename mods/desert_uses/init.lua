...
minetest.register_node("desert_uses:desert_furnace", {
	...
	on_construct = function(pos)
		...
		meta:set_string("infotext", "Печь из пустынника")
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if meta:get_float("fuel_time") < meta:get_float("fuel_totaltime") then
			...
			if cooked and cooked.item and meta:get_float("src_time") >= cooked.time then
				...
				if inv:room_for_item("dst",cooked.item) then
					...
					print("Нельзя сюда положить '"..cooked.item:to_string().."'")
				end
		...
		if meta:get_float("fuel_time") < meta:get_float("fuel_totaltime") then
			...
			meta:set_string("infotext","Печь активна: "..percent.."%")
            meta:set_string("percent",tostring(percent))
		...
		if fuel.time <= 0 then
			meta:set_string("infotext","Печь без топлива")
		...
		if cooked.item:is_empty() then
			if was_active then
				meta:set_string("infotext","Печь пуста")
				hacky_swap_node(pos,"desert_uses:desert_furnace")
				meta:set_string("formspec", default.furnace_inactive_formspec)
			end
			return
