...
if ttt~="" then minetest.debug("At " .. minetest.pos_to_string(p).." was punched "..ttt) end
    if (shared_autoban and check_ownership_once(p,puncher:get_player_name()))
...
minetest.debug("boom2 ".. puncher)
        ...
        for dx=-2,2 do
            for dz=-2,2 do
                ...
                --  if owne ~="" then
                --     minetest.debug("At ".. minetest.pos_to_string(p) .. " node " .. node.name .. " owner is " ..owne)
                --  end
                            ...
                            if shared_autoban ~= nil
                                then
                                    if check_ownership_once(p,puncher)
                                       then
                                          --  minetest.debug("At ".. minetest.pos_to_string(p) .." ".. node.name .. " destroyed by " .. puncher .. ", owner is: ".. owne)
                                            destroy(p)
                                       end
                                else
                                --destroy(p)
