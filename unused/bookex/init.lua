...
minetest.register_craftitem("bookex:book", newbook);

minetest.register_on_player_receive_fields(function ( player, formname, fields )
    if ((formname == "bookex:book") and fields and fields.text) then
        local stack = player:get_wielded_item();
        if (stack:get_name() and (stack:get_name() == "bookex:book")) then
