...
local get_specialInfo = function(name, specialty)
    ...
    "button[2,0;2,0.5;miner;Шахтёр]"..
    "button[2,.65;2,0.5;lumberjack;Дровосек]"..
    "button[2,1.3;2,0.5;digger;Копатель]"..
    "button[2,1.95;2,0.5;farmer;Фермер]"..
    "button[2,2.6;2,0.5;builder;Строитель]"..
    "list[current_player;main;0,3.5;9,3;9]"..
    "list[current_player;main;0,7;9,1;]"

    if iplus then
        formspec = formspec.."button[0,0;2,0.5;main;Назад]"
    else
...
minetest.register_chatcommand("spec", {
    description = "Показывает меню профессий",
...
minetest.register_on_joinplayer(function(player)
    ...
    minetest.after(0.5, function(name)
        ...
        for skill,num in pairs(specialties.players[name].skills) do
            ...
            if skill=='miner' then skill_rus='Шахтёр' end
            if skill=='lumberjack' then skill_rus='Дровосек' end
            if skill=='digger' then skill_rus='Копатель' end
            if skill=='farmer' then skill_rus='Фермер' end
            if skill=='builder' then skill_rus='Строитель' end
            player:hud_add({
                hud_elem_type = "text",
                position = {x=0, y=0.85+y},
                offset = {x=10, y=0},
...
function minetest.node_dig(pos, oldnode, digger)    
    ...
    if digger == nil then
       -- minetest.debug('digger is nil')
        return pos, oldnode, digger
    end
    if digger:get_wielded_item():is_empty() then
        --minetest.debug('there\'s NO wielded item')
        return pos, oldnode, digger     
    end     
